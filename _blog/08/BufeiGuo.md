...
3. Rename the <LastnameFirstname>`.Rmd` in your repo. Write at least 2-3 sentences describing the problems you encountered while following these instructions, how you solved them, and how you anticipate using this site in the future. 
    This is link to my site bufei2022.github.io
    I used the second method to build my website. There are few problems I encountered when I try to follow the instructions and make my first post
    1. My directory is not the same as that in step4, I got config.yaml not config.toml. Then I solve the problem by unclick the "convert the site config
    file to YAML" and "Convert all post metadata to YAML" option when create blogdown website.
    2. When I build my website and commit, the materials on "Bufei2022.github.io" does not seem to update as well. I searched stackover but could find a
    good solution. However, I found out that the update is not immediate, which means I have to wait several miniutes to see my updates.
    3. Include pictures and videos in post is most problematic. But I really want to share something about my cats, I figure this problem out anyway. 
    Stackover flow and few other websites suggestted that put the pictures in static file and call this picture by its relative location. A youtubde video
    also told me that I could click Addins and the click insert images, which is simple and easy. I couldn't fingure out how to include local videos in 
    blogdown, so I include youtube videos--just click share and select the option `embed`, the youtube will give me the code I needed to embed the
    selected video.
    
    I will use this site to share my cats and fishes (I have a female betta sorority, each betta has a name and different personality). Also for future
    job searches... if I could make this site look more academic, not just talking about my pets.
