---
author: "Yuhang Lin"
title: "A personal blogdown website"
layout: post
topic: "08"
...
1. Create a new GitHub repository named `<your-username>.github.io` (replacing `<your-username>` with your GitHub username). Initialize this repo with a readme.
    ...
    **Answer**: The repository can be found [here](https://github.com/YuhangTom/YuhangTom.github.io).
...
2. Set up your website using either the Simple Markdown Site or Blogdown instructions below. If you would like to use this repository to host a blog, you may find Blogdown to be a more convenient option, however, it is more complicated than the simple Rmarkdown site. 
    ...
    **Answer**:
    I would use `blogdown` in the following.
    My website is [here](https://yuhangtom.github.io/).
...
3. Rename the <LastnameFirstname>`.Rmd` in your repo. Write at least 2-3 sentences describing the problems you encountered while following these instructions, how you solved them, and how you anticipate using this site in the future. 
    ...
    **Answer**:
    I have some trouble obtaining the required file structures described in the steps below.
    By default,
    Rstudio is creating `config.yaml` instead of `config.toml`.
    I read Yihui's suggestions in [blogdown: Creating Websites with R Markdown, section 2.2](https://bookdown.org/yihui/blogdown/configuration.html),
    and untick `Convert the site config file to YAML` and get the desired file structures.
    
    Also,
    I tried to upload some external HTML without uploading the md/Rmd files,
    and it takes me several hours to figure out what static files are really doing by trial and error,
    as you may see from my commit history.
    It is a real mess there.
    Anyways,
    in the end,
    I created the post I wanted and also link those external HTML files to the website.
    
    I guess I would keep upload some knitted HTML files to my website as they are not that compatible with the simple google site,
    which I have been using for almost three years.
    HTML files in the google site need to be downloaded first before preview.
    The google site is more interactive and straightforward when adding and modifying things,
    but the GitHub page is easier for linking HTML files.
